...
async function adminRequest(request: Request, env: Env): Promise<Response> {
  ...
  if (request.method === "GET" && url.searchParams.get("view") === "change-password") {
    return adminChangePasswordPage();
  }
  if (request.method === "POST" && (isRoot || url.pathname === "/__admin") && url.searchParams.get("view") === "change-password") {
    const form = await request.formData();
...
function adminLandingPage(request: Request, adminUsername: string, accounts: WebdavAccount[], nextUuid: string, message: string): Response {
  ...
  return htmlResponse(`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>WebDAV 账户中心</title><style>${ADMIN_CSS}${FILES_CSS}</style><body><header class="topbar"><div class="topbar-inner"><div class="brand"><span class="brand-mark small">WD</span><span>WebDAV 账户中心</span></div><div class="topbar-right"><span class="status-dot">管理员：${escapeHtml(adminUsername)}</span><a class="text-link" style="color:#dcebe6" href="/?action=logout">退出登录</a></div></div></header><main class="dashboard"><section class="page-heading"><div><p class="eyebrow">ACCOUNT SELECTOR</p><h1>选择 WebDAV 账户</h1><p class="muted">进入账户后只能管理该账户自己的文件。</p></div></section>${message ? `<div class="notice success">${escapeHtml(message)}</div>` : ""}<section class="content-grid">${accountCards}${createForm}</section><p class="muted">${accounts.length}/2 个 WebDAV 账户</p></main><script>async function checkUuidAvailability(btn){var row=btn.closest('.uuid-row');var input=row.querySelector('input');var result=document.getElementById('uuid-check-result');var uuid=input.value.trim();result.className='uuid-result';result.textContent='检查中…';if(!/^[0-9]{6}$/.test(uuid)){result.textContent='UUID 必须是 6 位数字';result.className='uuid-result error';return false;}try{var res=await fetch('/?api=check-uuid&uuid='+encodeURIComponent(uuid));var data=await res.json();result.textContent=data.message;result.className='uuid-result '+(data.ok&&data.available?'success':'error');}catch(e){result.textContent='检查失败，请重试';result.className='uuid-result error';}return false;}</script></body></html>`);
}
...
async function adminAccountsPage(request: Request, env: Env, adminUsername: string): Promise<Response> {
  ...
  return htmlResponse(`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>账号管理</title><style>${ADMIN_CSS}${FILES_CSS}</style><body><header class="topbar"><div class="topbar-inner"><div class="brand"><span class="brand-mark small">WD</span><span>账号管理</span></div><div class="topbar-right"><a class="text-link" style="color:#dcebe6" href="/">返回管理中心</a><a class="text-link" style="color:#dcebe6" href="/?action=logout">退出登录</a></div></div></header><main class="dashboard"><section class="page-heading"><div><p class="eyebrow">ACCOUNT MANAGEMENT</p><h1>账号管理</h1><p class="muted">当前管理员：${escapeHtml(adminUsername)}。每个管理员最多拥有两个 WebDAV 账户。</p></div></section><section class="content-grid">${rows}${createForm}</section><section class="config-card admin-account-card"><div class="card-heading"><div><p class="eyebrow">NEW ADMIN</p><h2>创建管理员账户</h2></div></div><form method="post" action="/?view=accounts" class="config-form"><input type="hidden" name="action" value="create-admin"><label>管理员账户<input name="adminUsername" required></label><label>管理员密码<input name="adminPassword" type="password" minlength="8" required></label><button class="primary-button" type="submit">创建管理员</button></form></section></main></body></html>`);
}
...
function adminChangePasswordPage(message = "", error = ""): Response {
  return htmlResponse(`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>修改密码</title><style>${ADMIN_CSS}</style><body><header class="topbar"><div class="topbar-inner"><div class="brand"><span class="brand-mark small">WD</span><span>修改密码</span></div><div class="topbar-right"><a class="text-link" style="color:#dcebe6" href="/">返回首页</a></div></div></header><main class="login-shell"><section class="login-panel"><div class="brand-mark">WD</div><p class="eyebrow">ACCOUNT SECURITY</p><h1>修改密码</h1><p class="muted">为保障账户安全，修改密码前需要先验证当前密码。</p>${message ? `<p class="notice success">${escapeXml(message)}</p>` : ""}${error ? `<p class="error">${escapeXml(error)}</p>` : ""}<form method="post" action="/?view=change-password"><input type="hidden" name="action" value="change-own-password"><label>当前密码<input name="currentPassword" type="password" autocomplete="current-password" required></label><label>新密码<input name="newPassword" type="password" autocomplete="new-password" minlength="8" placeholder="至少 8 位" required></label><label>确认新密码<input name="confirmPassword" type="password" autocomplete="new-password" minlength="8" required></label><button class="primary-button" type="submit">保存新密码</button></form></section></main></body></html>`);
}
...
function htmlResponse(body: string): Response {
  ...
  const withLogout = renderedBody.replaceAll("返回账户选择", "返回用户管理").replace('<span class="status-dot">服务在线</span>', '<span class="status-dot">服务在线</span><a class="text-link" style="color:#dcebe6;margin-left:16px" href="/?action=logout">退出当前账户</a>');
  ...
  let polishedBody = withAccountTools.replace("</style>", `${UI_POLISH_CSS}${DARK_MODE_CSS}${TABLE_POLISH_CSS}${FORM_LAYOUT_CSS}${TOPBAR_LAYOUT_CSS}${DARK_TEXT_CSS}${LOGIN_DARK_CSS}${FILE_ACTION_ALIGNMENT_CSS}${THEME_TOGGLE_CSS}</style>`);
...
async function adminLogsPage(env: Env): Promise<Response> {
  ...
  return htmlResponse(`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>访问日志</title><style>${ADMIN_CSS}${LOGS_CSS}</style><body><header class="topbar"><div class="topbar-inner"><div class="brand"><span class="brand-mark small">WD</span><span>访问日志</span></div><div class="topbar-right"><a class="text-link inverse" href="/">返回管理中心</a></div></div></header><main class="dashboard"><section class="page-heading"><div><p class="eyebrow">ACCESS LOG</p><h1>访问日志</h1><p class="muted">最近 ${recentLogs.length} 条记录（保留 ${LOG_RETENTION_DAYS} 天）</p></div></section><section class="data-table-wrap"><table class="data-table"><thead><tr><th>时间</th><th>方法</th><th>路径</th><th>状态</th><th>大小</th><th>IP</th><th>用户</th></tr></thead><tbody>${logRows || '<tr><td colspan="7">暂无日志</td></tr>'}</tbody></table></section></main></body></html>`);
}
...
var FILE_ACTION_ALIGNMENT_CSS = `.file-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;align-items:stretch}.file-action-card{min-width:0;padding:18px;background:var(--card-bg);border:1px solid var(--card-border);border-radius:7px;box-shadow:var(--card-shadow)}.file-action-heading{display:flex;flex-direction:column;gap:5px;margin-bottom:14px;color:var(--text-primary)}.file-action-heading span{color:var(--text-secondary);font-size:12px}.file-action-card form{display:grid;grid-template-columns:minmax(0,1fr) 116px;align-items:center;gap:8px;width:100%;min-width:0}.file-action-card input[type=file],.file-action-card input[name=name]{display:block;width:100%;min-width:0;height:44px;line-height:42px;padding:0 10px;overflow:hidden;text-overflow:ellipsis}.file-action-card input[type=file]::file-selector-button{height:42px;margin-right:8px;padding:0 10px;border:0;border-right:1px solid var(--input-border);background:var(--table-header-bg);color:var(--text-primary);font:inherit}.file-action-card button{width:116px;min-width:116px;height:44px;min-height:44px;padding:0 10px;margin:0;white-space:nowrap}@media(max-width:720px){.file-actions{grid-template-columns:1fr}.file-action-card form{grid-template-columns:minmax(0,1fr) 116px}}`;


const THEME_SCRIPT = `<script>(function(){var s=localStorage.getItem('cf-webdav-theme');var d=window.matchMedia('(prefers-color-scheme:dark)').matches;if(s==='dark'||(!s&&d)){document.documentElement.setAttribute('data-theme','dark');var t=document.querySelectorAll('.theme-toggle');for(var i=0;i<t.length;i++)t[i].textContent='\\u2600\\uFE0F'}})();function toggleTheme(){var h=document.documentElement;var dark=h.getAttribute('data-theme')==='dark';if(dark){h.removeAttribute('data-theme');localStorage.setItem('cf-webdav-theme','light')}else{h.setAttribute('data-theme','dark');localStorage.setItem('cf-webdav-theme','dark')}var t=document.querySelectorAll('.theme-toggle');for(var i=0;i<t.length;i++)t[i].textContent=dark?'\\u{1F319}':'\\u2600\\uFE0F'}</script>`;
