...
function htmlResponse(body: string): Response {
  ...
  let polishedBody = withAccountTools.replace("</style>", `${UI_POLISH_CSS}${DARK_MODE_CSS}${TABLE_POLISH_CSS}${FORM_LAYOUT_CSS}${DARK_TEXT_CSS}</style>`);
...
async function adminLogsPage(env: Env): Promise<Response> {
  ...
  return htmlResponse(`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>访问日志</title><style>${ADMIN_CSS}${LOGS_CSS}</style><body><header class="topbar"><div class="topbar-inner"><div class="brand"><span class="brand-mark small">WD</span><span>访问日志</span></div><a class="text-link inverse" href="/">返回管理中心</a></div></header><main class="dashboard"><section class="page-heading"><div><p class="eyebrow">ACCESS LOG</p><h1>访问日志</h1><p class="muted">最近 ${recentLogs.length} 条记录（保留 ${LOG_RETENTION_DAYS} 天）</p></div></section><section class="data-table-wrap"><table class="data-table"><thead><tr><th>时间</th><th>方法</th><th>路径</th><th>状态</th><th>大小</th><th>IP</th><th>用户</th></tr></thead><tbody>${logRows || '<tr><td colspan="7">暂无日志</td></tr>'}</tbody></table></section></main></body></html>`);
}
...
async function adminTrashPage(env: Env, accountUsername: string): Promise<Response> {
  ...
  return htmlResponse(`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>回收站</title><style>${ADMIN_CSS}${TRASH_CSS}</style><body><header class="topbar"><div class="topbar-inner"><div class="brand"><span class="brand-mark small">WD</span><span>回收站</span></div><a class="text-link inverse" href="/?view=account&account=${encodeURIComponent(accountUsername)}">返回账户管理</a></div></header><main class="dashboard"><section class="page-heading"><div><p class="eyebrow">TRASH</p><h1>回收站</h1><p class="muted">账户：${escapeHtml(accountUsername)}。已删除的文件将在 ${TRASH_RETENTION_DAYS} 天后自动清理</p></div></section><section class="data-table-wrap"><table class="data-table"><thead><tr><th>原路径</th><th>类型</th><th>大小</th><th>删除时间</th><th>操作</th></tr></thead><tbody>${trashRows || '<tr><td colspan="5">回收站为空</td></tr>'}</tbody></table></section>${trashItems.length > 0 ? `<form method="post" action="/?view=trash&account=${encodeURIComponent(accountUsername)}" class="empty-form"><input type="hidden" name="action" value="empty"><input type="hidden" name="accountUsername" value="${escapeHtml(accountUsername)}"><button type="submit" class="empty-btn" onclick="return confirm('确定要清空回收站吗？此操作不可恢复！')">清空回收站</button></form>` : ""}</main></body></html>`);
}
